...
- **`AttributeSheet`:** Aggregates a dynamic collection of attributes enforcing duplication and initialization invariants.
- **`Race` & `CharacterClass`:** Treat core properties (such as racial stat bonuses or Hit Dice constraints) as immutable facts.
- **`AbilityDefinition`:** Normalizes identity based strictly on name, acting as the key structural reference for custom properties across systems.

### 3. Dual-Mode Flexibility

Business rules are dynamically bounded using the `CharacterMode` enum (`DnD5e` or `Custom`). The Character aggregate securely enforces safe instantiation states out of the box with safe factories: `Character.CreateDnD5e(...)` natively seeds standardized arrays of attributes bound rigidly to limitations, whereas `Character.CreateCustom(...)` processes explicitly built agnostic dictionaries enabling absolute custom behaviors mapped towards `AbilityDefinition`.
