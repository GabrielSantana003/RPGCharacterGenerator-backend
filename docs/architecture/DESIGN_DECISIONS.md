...
| `AbilityDefinition` | **Value Object** | Uniquely isolates stat identification to explicit normalized name strings enabling system-agnostic attribute creation securely away from primitive obsessions. |
...
- Aggregate root factories strictly partition behaviors. `Character.CreateDnD5e()` restricts values 1-30 calculating modifiers internally natively clamping limits; `Character.CreateCustom()` defers constraints entirely expecting safely built `AttributeSheet` objects containing unique `AbilityDefinition` inputs allowing infinite homebrew scaling seamlessly natively without schema changes.
